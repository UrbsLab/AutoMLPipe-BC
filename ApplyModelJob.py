...
def job(datasetFilename,full_path,class_label,instance_label,categorical_cutoff,sig_cutoff,cv_partitions,scale_data,impute_data,do_LR,do_DT,do_RF,do_NB,do_XGB,do_LGB,do_SVM,do_ANN,do_ExSTraCS,do_eLCS,do_XCS,do_GB,do_KN,primary_metric,data_path,match_label,plot_ROC,plot_PRC,plot_metric_boxplots,export_feature_correlations,jupyterRun):
    ...
    #Confirm that all featuers in original training data appear in replication datasets
    ...
    #ExploratoryAnalysis
    ExploratoryAnalysisJob.makeDataFolders(repData,full_path,'applymodel/'+apply_name) #Arguments changed to send to correct locations
    ...
    result_table,metric_dict = primaryStats(algorithms,cv_partitions,full_path,apply_name,instance_label,class_label,abbrev,colors,plot_ROC,plot_PRC,jupyterRun,evalDict,repData)
...
def primaryStats(algorithms,cv_partitions,full_path,apply_name,instance_label,class_label,abbrev,colors,plot_ROC,plot_PRC,jupyterRun,evalDict,repData):
    #Main Ops
    result_table = []
    metric_dict = {}
    for algorithm in algorithms:
        ...
        for cvCount in range(0,cv_partitions):
            results = evalDict[abbrev[algorithm]]
