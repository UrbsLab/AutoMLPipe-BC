...
License: GPL 3.0
Description: Phase 12 of AutoMLPipe-BC (Optional)- This 'Main' script manages Phase 12 run parameters, and submits job to run locally (to run serially) or on a linux computing
             cluster (parallelized). This script runs FileCleanupJob.py which deletes all temporary files in pipeline output folder. This script is not necessary to run, but
          serves as a convenience to reduce space and clutter.
Sample Run Command (Linux cluster parallelized with all default run parameters):
    python FileCleanupMain.py --data-path /Users/robert/Desktop/Datasets --out-path /Users/robert/Desktop/outputs --exp-name myexperiment1
Sample Run Command (Local/serial with with all default run parameters):
    python FileCleanupMain.py --data-path /Users/robert/Desktop/Datasets --out-path /Users/robert/Desktop/outputs --exp-name myexperiment1 --run-parallel False
"""
...
def main(argv):
    ...
    print(str(job_counter)+ " job submitted in Phase 12")

def submitLocalJob(experiment_path,data_path):
    """ Runs FileCleanupJob.py locally, once. """
...
def submitClusterJob(experiment_path,data_path,reserved_memory,maximum_memory,queue):
    """ Runs FileCleanupJob.py once. Runs on a linux-based computing cluster that uses an IBM Spectrum LSF for job scheduling."""
    job_ref = str(time.time())
    job_name = experiment_path + '/jobs/Clean_' + job_ref + '_run.sh'
    ...
    sh_file.write('python ' + this_file_path + '/FileCleanupJob.py ' + experiment_path +" "+ str(data_path)+ '\n')
    sh_file.close()
